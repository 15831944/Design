...
{
	for (int i = 0; i < beam->sectionNumber; i++)//[]改成for each(BeamSection& curSection in beam->sections)行不行
	{//遍历梁的所有验算截面
...
(Section* section
, BeamSection::E_BeamSectionLocation sectionLocation
...
void DesignBeam::setDesignParameterN(BeamSection::E_BeamSectionLocation sectionLocation)
{
	ξb = additionData.ξb;
	γRE = 1.0;
...
void DesignBeam::setDesignParameterAD(BeamSection::E_BeamSectionLocation sectionLocation)
{
...
void DesignBeam::setDesignParameterE(BeamSection::E_BeamSectionLocation sectionLocation)
{
	ξb = additionData.ξb_E;
	γRE = additionData.γRE_M;
	switch (sectionLocation)
	{
	case BeamSection::E_BSL_LR:
		ρmin = additionData.ρmin_E_LR;
		break;
	case BeamSection::E_BSL_M:
...
void DesignBeam::designM(Section* section, const ForceEffect::CombineForceData& forceData, BeamSection::ResultFC& resultFC)
{
	switch (section->getType())
	{
	case Section::E_Section::E_S_RECT_SECTION:
	...
	case Section::E_Section::E_S_CIRCLE_SECTION:
	{
...
void DesignBeam::designV(Section* section, const ForceEffect::CombineForceData& forceData, BeamSection::ResultFC& resultFC)
{
...
void DesignBeam::setParameter()
{
	...
	if (beam->beamType == Beam::E_BeamType::E_BT_TRANSFER_BEAM)
	{
...
void DesignBeam::setParameterE()
{
	...
	{
	//转换梁
	case Beam::E_BeamType::E_BT_TRANSFER_BEAM:
		switch (beam->Nfb_gz)
		{
	...
	//其他梁
	case Beam::E_BeamType::E_BT_BEAM:
